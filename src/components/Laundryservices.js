...
const LaundryServices = () => {
  ...
  const categoryData = ['Select Category', 'Iron & Fold', 'Wash & Iron', 'Wash & Fold', 'Dry Cleaning', 'Dry Cleaning','Emergency Service','Subscription Based'];
  ...
  const handleCityChange = (selectedCity) => {
   ...
   const handlePGSearch = () => {
    ...
    router.push(`/Laundry?title=${encodeURIComponent(category)}&state=${state?.name}&city=${city?.name}`);
  };
  return (
      ...
      <select   value={category}
         ...
         {categoryOption}
                  ...
                  {state && (
                        ...
                        <button onClick={handlePGSearch} className="border-2 w-56 h-12 mt-4 md:mt-0 border-emerald-600 text-black px-4 py-2 rounded-md text-1xl font-medium hover:border-white text-white bg-emerald-600 transition duration-300 ">Search</button>
                </div>
              </div>
              </div>
  )
