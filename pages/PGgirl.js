...
const Filter1 = () => {
  ...
  const { Boys, Girls, fabric, crocodile, wool, SingleRoom, DoubleSharingRoom, TripleSharingRoom, SingleAcRoom,DoubleSharingAcRoom,TripleSharingAcRoom, luxesignatire, luxelondon } = check;
  ...
  const applyFilters = (e) => {
    ...
    const filteredData = fetchedData.filter((item) => {
      ...
      // Filter based on price within roomTypes
  ...
  const maxPrice = 50000; // Define max price
    ...
    useEffect(() => {
      const fetchData = async () => {
        try {
          ...
          const collectionRef = firebase.firestore().collection('pgdetail').where('Verified', '==', 'true').where('subcat', '==', 'Girls');
...
Experience the difference with Arene Services - where quality meets convenience, and your satisfaction is our priority. Join us today and discover the perfect PG solution tailored to meet your needs across India.</p>
                ...
                <p className=" text-xl leading-5 text-gray-600 font-medium">{TotalData} PG</p>
