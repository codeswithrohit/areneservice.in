...
const Laundry = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const db = firebase.firestore();
        const productRef = db.collection('Laundry');
  ...
  return (
    <div className='min-h-screen  bg-white dark:bg-white'>
      <AdminNavbar />
      <div className=" lg:ml-64 text-center  md:p-4 bg-white dark:bg-white ">
        {showAllInputFormats ? (
          <div>
                  ...
                  <tr key={item.id} class="hover:bg-gray-50">
                    ...
                    <td class="px-6 py-4 text-base">
                      {item.GarmentTypes && item.GarmentTypes.map((type, idx) => (
                        <div key={idx}>
                          <span>{type.tenure} </span> - <span>{type.noofgarments} Garments</span> - <span>₹ {type.price}</span>
                        </div>
                      ))}
                    </td>
                    <td class="px-6 py-4">
