...
const PGData = () => {
    ...
    const handleVerificationChange = async (id, Verified) => {
        try {
            ...
            setPgdata(prevState => prevState.map(item => item.id === id ? { ...item, Verified: Verified === 'true' } : item));
            if (Verified === 'true') {
            ...
            console.log(`Verification status for PG with ID ${id} changed to ${Verified}`);
            
            // Show toast notification
            toast.success(`Verification status for PG with ID ${id} changed to ${Verified === 'true' ? 'Approved' : 'Unapproved'}`);
        } catch (error) {
    ...
    // Get current items
    const filteredData = filter === 'approved' ? pgdata.filter(item => item.Verified) :
                        filter === 'unapproved' ? pgdata.filter(item => !item.Verified) :
                        pgdata;
    ...
    return (
        <div className="min-h-screen bg-gray-100">
            ...
            <div className="lg:ml-64 p-8">
                <h1 className="text-3xl font-bold mb-8">PG Data</h1>
                    ...
                    <div className="overflow-x-auto">
                        <table className="min-w-full bg-white shadow-md rounded-md overflow-hidden">
                            <thead className="bg-gray-200 text-gray-700">
                                <tr>
                                    <th className="py-3 px-4 text-left">PG Name</th>
                                    <th className="py-3 px-4 text-left">PG Owner</th>
                            ...
                            <tbody>
                                {currentItems.map((item, index) => (
                                    <tr key={index} className={`${index % 2 === 0 ? 'bg-gray-100' : 'bg-white'}`}>
                                        <td className="py-3 px-4">{item.PGName}</td>
