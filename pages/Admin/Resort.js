...
const Resortdata = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                const galleryRef = db.collection('Resortdetail');
    ...
    const handleVerificationChange = async (id, Verified) => {
        try {
            const db = firebase.firestore();
            const RentRef = db.collection('Resortdetail').doc(id);
            await RentRef.update({ Verified: Verified });
            // Update the local state to reflect the change immediately
            setResortData(prevState => prevState.map(item => item.id === id ? { ...item, Verified: Verified === 'true' } : item));
            if (Verified === 'true') {
            ...
            console.log(`Verification status for Resort with ID ${id} changed to ${Verified}`);
            
            // Show toast notification
            toast.success(`Verification status for Resort with ID ${id} changed to ${Verified === 'true' ? 'Approved' : 'Unapproved'}`);
        } catch (error) {
    ...
    // Get current items
    const filteredData = filter === 'approved' ? resortdata.filter(item => item.Verified) :
                        filter === 'unapproved' ? resortdata.filter(item => !item.Verified) :
                        resortdata;
    ...
    return (
        <div className="min-h-screen bg-gray-100">
            ...
            <div className="lg:ml-64 p-8">
                <h1 className="text-3xl font-bold mb-8">Resort Data</h1>
                    ...
                    <div className="overflow-x-auto">
                        <table className="min-w-full bg-white shadow-md rounded-md overflow-hidden">
                            <thead className="bg-gray-200 text-gray-700">
                                <tr>
                                    <th className="py-3 px-4 text-left">Resort Name</th>
                                    <th className="py-3 px-4 text-left">Resort Owner</th>
                            ...
                            <tbody>
                                {currentItems.map((item, index) => (
                                    <tr key={index} className={`${index % 2 === 0 ? 'bg-gray-100' : 'bg-white'}`}>
                                        <td className="py-3 px-4">{item.ResortName}</td>
