...
const Buydata = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                const galleryRef = db.collection('buydetail');
    ...
    const handleVerificationChange = async (id, Verified) => {
        try {
            const db = firebase.firestore();
            const pgRef = db.collection('buydetail').doc(id);
            await pgRef.update({ Verified: Verified });
            // Update the local state to reflect the change immediately
            setBuyData(prevState => prevState.map(item => item.id === id ? { ...item, Verified: Verified === 'true' } : item));
            if (Verified === 'true') {
            ...
            console.log(`Verification status for Buy Property with ID ${id} changed to ${Verified}`);
            
            // Show toast notification
            toast.success(`Verification status for Buy Property with ID ${id} changed to ${Verified === 'true' ? 'Approved' : 'Unapproved'}`);
        } catch (error) {
    ...
    // Get current items
    const filteredData = filter === 'approved' ? buydata.filter(item => item.Verified) :
                        filter === 'unapproved' ? buydata.filter(item => !item.Verified) :
                        buydata;
    ...
    return (
        <div className="min-h-screen bg-gray-100">
            ...
            <div className="lg:ml-64 p-8">
                <h1 className="text-3xl font-bold mb-8">Buy/Sell Property Data</h1>
                        ...
                        <table className="min-w-full bg-white shadow-md rounded-md overflow-hidden">
                            <thead className="bg-gray-200 text-gray-700">
                                <tr>
                                    ...
                                    <th className="py-3 px-4 text-left">Property Owner</th>
                            ...
                            <tbody>
                                {currentItems.map((item, index) => (
                                    <tr key={index} className={`${index % 2 === 0 ? 'bg-gray-100' : 'bg-white'}`}>
                                        <td className="py-3 px-4">{item.Propertyname}</td>
                                        <td className="py-3 px-4">{item.Owner}</td>
                                        <td className="py-3 px-4">{item.subcat}</td>
