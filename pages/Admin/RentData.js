...
const Rentdata = () => {
    ...
    const handleVerificationChange = async (id, Verified) => {
        try {
            const db = firebase.firestore();
            const RentRef = db.collection('rentdetail').doc(id);
            await RentRef.update({ Verified: Verified });
            // Update the local state to reflect the change immediately
            setRentData(prevState => prevState.map(item => item.id === id ? { ...item, Verified: Verified === 'true' } : item));
            if (Verified === 'true') {
            ...
            console.log(`Verification status for Rent with ID ${id} changed to ${Verified}`);
            
            // Show toast notification
            toast.success(`Verification status for Rent with ID ${id} changed to ${Verified === 'true' ? 'Approved' : 'Unapproved'}`);
        } catch (error) {
    ...
    // Get current items
    const filteredData = filter === 'approved' ? rentdata.filter(item => item.Verified) :
                        filter === 'unapproved' ? rentdata.filter(item => !item.Verified) :
                        rentdata;
    ...
    return (
        <div className="min-h-screen bg-gray-100">
            ...
            <div className="lg:ml-64 p-8">
                <h1 className="text-3xl font-bold mb-8">Rent Property Data</h1>
